...
\begin{definitionenv}
    ...
    where $\varphi_X:F(X)\rightarrow G(X)$ is a morphism (in $\mathcal{D}$) such that from any morphism $f:X\rightarrow Y$ in $\mathcal{C}$, the following diagram is commutative:
    \begin{center}
    \begin{tikzcd}
...
\begin{proofenv}
    ...
    \begin{center}
    \begin{tikzcd}
\mathcal{C}(X,A)   &\displaystyle \mathcal{C}(X,B) \arrow[l, swap, "h_X(\mu)"] \\
F(A) \arrow[u, "\alpha(x)_A"]& F(B) \arrow[l,"F(\mu)"] \arrow[u,  swap,"\alpha(x)_B"]
\end{tikzcd}
\end{center}
In fact, 
...
\end{proofenv}
\begin{notationenv}
    Let $\mathcal{C}$ be a category, and $f:Y\rightarrow X$ be a morphism in $\mathcal{C}$.
    $$\begin{array}{rcl}
