...
\end{proofenv}
\begin{propositionenv}
    Let $\varphi:\RR\longrightarrow \RR$ be an increasing right continuous mapping. Let $\mathcal{C}\subseteq \mathscr{P}(\RR)$ consisting of $\interval[open left]{a}{b}$, $a,b\in \RR$, $a<b$, $S\subseteq \RR^\RR$ vector subspace generated by $\mathbbm{1}_A$, $A\in \mathcal{A}\in \mathcal{C}$. $I_\varphi: S\longrightarrow \RR$ be $\RR$-linear mapping, $I_\varphi(\mathbbm{1}_{\interval[open left]{a}{b}})\coloneq \varphi(b)-\varphi(a).$ $\mathcal{A}\subseteq \mathscr{P}(\RR)$. 
    $$ \mathcal{A}\coloneq \{A\mid A=C_1\cup \cdots\cup C_n,\ C_1,\cdots,C_n\in \mathcal{C} \}.$$
    (1) Let $\varepsilon>0$, $A\in \mathcal{A}$, $A=\varnothing$. There exists $B\in \mathcal{A}$, $\varnothing\neq \overline{B}\subseteq A$ and $I_\varphi(\mathbbm{1}_A)-I_\varphi(\mathbbm{1}_B)\le \varepsilon$.
    \newline
...
\begin{proofenv}
    ...
    \begin{align*}
        ...
        =&\{\omega\in \Omega\mid \limsup_{n\rightarrow +\infty} \dd(f_n(\omega),Y)\}\\
    ...
    $$ \{\omega\in \Omega\mid \limsup_{n\rightarrow+\infty} \dd(f_n(\omega,Y))\}\in \mathcal{A}.$$
\end{proofenv}
\newpage
...
\end{proofenv}
\begin{definitionenv}
    Let $\Omega$ be a set and $\mathcal{H}$ be a family of bounded mappings from $\Omega$ to $\RR_{\ge 0}$. We say that $\mathcal{H}$ is a $\lambda$-family if the following conditions are satisfied:
    \newline
...
\begin{proofenv}
    ...
    In fact, by the lemma $\dis f^n=\sup_{a\in \QQ_{>0}}\left(\sup\{na^{n-1}f-(n-1)a^n\mathbbm{1}_\Omega,0\}\right)\in \mathcal{H}$.

    \quad Let $\mathcal{A}=\{A\in\mathscr{P}(\Omega)\mid \mathbbm{1}_{A}\in \mathcal{H}\}$.
    \newline
    ...
    Therefore $\mathcal{A}$ is a $\sigma$-algebra contained in $\sigma(H)$.
...
\begin{propositionenv}
    ...
    Then $\mathcal{C}$ is a semiring and $\mu|_e$ is $\sigma$-additive.
\end{propositionenv}
\begin{proofenv}
...
\begin{definitionenv}
    Let $(\Omega,\mathcal{A},\mu)$ be a measure space. We say that $A\in \mathscr{P}(\Omega)$ is $\mu$-negligeable if there is $B\in \mathcal{A}$, $\mu(B)=0$ and $A\subseteq B$.
    (If $A\in\mathcal{A}$ and is $\mu$-negligeable, then $\mu(A)=0$).
\end{definitionenv}
\begin{propositionenv}
    If $f\in \mathcal{L}^{\Omega,\mathcal{A}, \mu}$ and $A\in \mathcal{A}$, $\mu(A)=0$, then $\int_{\Omega} \mathbbm{1}_A f\dd\mu=0$.
\end{propositionenv}
\begin{proofenv}
...
\begin{proofenv}
    ...
    \newline
    (2) $\dis f=\lim_{n\rightarrow +\infty}\sum_{k=0}^{\RR^n} \frac{k}{2^n}\mathbbm{1}_{\{\frac{k}{2^n}\le f\le \frac{k+1}{2^n}\}}+n\mathbbm{1}_{\{f\le n\}}$. We may assume that $f$ is bounded.
    Let 
    $$\mathcal{H}=\{f:X\times Y\longrightarrow \RR_{\ge 0}\mid f \text{ bounded }, f(x,\cdot) \text{ is } A_Y\text{-measurable}, \forall x\in X\}.$$
...
\begin{proofenv}
    ...
    So the same equality holds for any bounded non-negative $\mathcal{A}_X\otimes \mathcal{A}_Y$-measurable mappings $X\times Y\longleftrightarrow \RR_{\ge 0}$.
...
\begin{theoremenv}[Fubini]
    ...
    $$ \int_{X\times Y}\dd (\mu_X\otimes \mu_Y)=\int_{X}\int_{Y} f(x,y)\mu_{Y}(\dd y)\mu_{X}(\dd x).$$
\end{theoremenv}
\begin{proofenv}
    \ \newline
