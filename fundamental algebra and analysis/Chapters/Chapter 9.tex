...
\begin{propositionenv}
    Let $(X,d)$ be a metric space and $(x_n)_{n\in \NN}$ be a sequence in $X$. If 
    $$\sum_{n\in \NN}\dd(x_n,x_{n+1})<+\infty,$$
    then $(x_n)_{n\in \NN}$ is a Cauchy sequence.
\end{propositionenv}
\begin{proofenv}
    Let $N\in \NN$. If $(n,m)\in \NN^2$, $n>m$, by the triangle inequality, 
    ...
    $$ \lim_{(n,m)\in \NN_{\ge N}^2} \dd(x_n,x_m)=0.$$
    Let $(a_n)_{n\in\NN}\in \RR^\NN$. If $\dis \left(\sum_{k=0}^{n}a_k\right)_{n\in\NN}$ converges to some $l$ in $\RR$. Then, $l-\sum_{k=0}^{N-1}a_k$ converges to $0$. If $a_k\le 0$ for any $k\in\NN$, $l-\sum_{k=0}^{N-1}a_k=\sum_{k=N}^{+\infty}a_k$.
...
\begin{theoremenv}[Root test of Cauchy]
    ...
    $$ r=\limsup_{n\rightarrow_\infty}\pl x_n\pl^{\frac{1}{n}}\in [0,+\infty]$$
    If $r<1$, then $\sum_{n\in \NN}x_n$ converges absolutely.
    \newline
    If $r>1$, then $\sum_{n\in \NN}x_n$ diverges.
\end{theoremenv}
\begin{lemmaenv}
    If a series $\sum_{n\in \NN}x_n$ converges, then $\lim_{n\rightarrow+\infty}\pl x_n\pl =0$.
\end{lemmaenv}
\begin{proofenv}[of lemma]
    $$ \pl x_n\pl =\pl \sum_{k=0}^{n}x_k-\sum_{k=0}^{n-1}x_k\pl .$$
    Since $\sum_{k}^n x_k$ converges to some $l\in E$.
...
\begin{proofenv}[of theorem]
    ...
    So,
    $$ \sum_{n\ge N}\pl x_n\pl \le \sum{n\ge N}\alpha^n=\frac{\alpha^N}{1-\alpha}<+\infty.$$
...
$$ \exists N\in \NN, \forall x\in \NN_{N}, \pl x_{n+1}\pl \alpha \pl x_n\pl\le \alpha^{n+1-N}\pl x_N\pl.$$
Thus,
...
\end{definitionenv}
\begin{definitionenv}
    Let $f:X\longrightarrow X$ be a mapping. If $x\in X$ id such that $f(x)=x$, we say that $x$ is a \textbf{fixed point} of $f$.
\end{definitionenv}
...
\end{proofenv}
\begin{theoremenv}
    Let $(X,\dd_X)$ and $(Y,\dd_Y)$ be two metric spaces, $(f_n)_{n\in\NN}$ sequence of uniformly continuous mappings from $X$ to $Y$. Suppose that $(f_n)_{n\in\NN}$ converges uniformly to $f:X\longrightarrow Y$. Then $f$ is uniformly continuous.
\end{theoremenv}
\begin{proofenv}
...
\begin{theoremenv}
    ...
    (3) There exists $N\in \NN$ and mapping $\delta: U\longrightarrow \RR_{\ge 0}$ such that $\lim_{x\rightarrow p}\delta(x)=0$ and for any $n\in\NN_{\ge N}$, any $x\in U$,
...
\begin{propositionenv}
    ...
    \begin{enumerate}
        \item $\forall n\in \NN_{\ge N}$, $\lim_{x\rightarrow p}\delta_(x)=\delta_n(p)=0$.
    ...
    \end{enumerate}
    Let $f$ and $g$ be limits of $\sum_{n\in\NN}f_n$ and $\sum_{n\in \NN}\DD f_n$ respectively. Then $f$ is differentiable at $p$ and $\DD f=g$.
\end{propositionenv}
\begin{propositionenv}
