...
\end{proofenv}
\begin{notationenv}
    Let $(M,*)$ be a monoid, $x\in M$, $f:(\NN,+)\rightarrow (M,*)$ be the unique homomorphism of monoid, such that $f(1)=x$. $\forall n\in \NN$, we denote by $x^{*n}$ the element $f(n)\in M$, $x^{*0}=e_M$, $x^{*(n+m)}=x^{*n}*x^{*m}$.
    \newline
...
\begin{propositionenv}
    ...
    \newline
    (1) If $x*y=y*x$, then $\forall(n,m)\in \NN^2$,
     $$x^{*n}*y^{*m}=y^{*m}*x^{*n}$$ 
     $$(x*y)^{*n}=x^{*n}*y^{*n}$$
    \newline
    (2) If $x\in M$, $\iota(x^{*n})=\iota(x)^{*n}$ and $\forall (n,m)\in \NN^2$, with $n\ge m$,
    $$x^{*(n-m)}=x^{*n}*\iota(x)^{*m}$$
